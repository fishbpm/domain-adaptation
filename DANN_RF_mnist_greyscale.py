...
#mnist_valid = (mnist.valid.images > 0).reshape(10000, 28, 28, 1).astype(np.uint8) * 255
#mnist_valid = np.concatenate([mnist_valid, mnist_valid, mnist_valid], 3)

test_size = len(mnist_test)

mnistm = pkl.load(open('mnistm_data.pkl', 'rb'))
mnistm_train_rgb = mnistm['train'][:TRAIN_SIZE]
mnistm_test_rgb = mnistm['test']
#mnistm_valid = mnistm['valid'][:,:,:,:1]

mnistm_train = np.zeros([TRAIN_SIZE, 28, 28, 1], np.uint8)
mnistm_test = np.zeros([test_size, 28, 28, 1], np.uint8)

for sample, image in enumerate(mnistm_train_rgb):
    image = Image.fromarray(image, 'RGB')
    image = np.asarray(image.convert('L'), dtype="uint8")
    mnistm_train[sample] = image.reshape(28, 28, 1)

for sample, image in enumerate(mnistm_test_rgb):
    image = Image.fromarray(image, 'RGB')
    image = np.asarray(image.convert('L'), dtype="uint8")
    mnistm_test[sample] = image.reshape(28, 28, 1)

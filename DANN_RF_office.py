...
EPOCHS = 10 #baseline #epochs, as per MNIST data
...
domains = ('dslr','webcam','amazon') #source / target / unseen
...
source_train, ys_train = loadimages(domains[0], 'train')
source_test, ys_test = loadimages(domains[0], 'test')
target_train, yt_train = loadimages(domains[1], 'train')
target_test, yt_test = loadimages(domains[1], 'test')
unseen_train, yu_train = loadimages(domains[2], 'train')
unseen_test, yu_test = loadimages(domains[2], 'test')
...
#for domain, images in enumerate(experiment):
#    experiment[domain] = (experiment[domain] - pixel_mean) / 255
...
#        fitnesses[gen] = np.mean(fitness)
    ...
    """----------------track current model accuracies for the full domain----------------------
       ------------this is performed at the end of every MNIST equivalent epoch-------------------"""

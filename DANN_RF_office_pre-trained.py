...
def decolourise(images):
    grey_images = np.zeros([len(images), 64, 64, 1], np.uint8)
...
batch_size = int(len(source_train)/round(len(source_train)/128, 0))
full_model.fit(source_train, ys_train_hot,
                    batch_size=batch_size, epochs=20,
                    verbose=0, validation_data=(source_test, ys_test_hot))
score = full_model.evaluate(source_test, ys_test_hot, verbose=0)
...
weights = K.get_trainable_weights(model, FREEZE)
#dim = len(weights)
